...
export class Server {
  ...
  async sendToPageEventBus(object) {
    await this.window.webContents.send("eventbus", {
      data: object,
  ...
  async onRequestRunCommandLineCommand({ command, args, prependCurrentPath }) {
    ...
    command = command
      .replaceAll("resources/app.asar/", "")
      .replaceAll("Resources/app.asar/", "");
    this.logConsole("Running command line Server", command, args);

    // NOTE: FOR LINUX AND MACOS ONLY
    if (this.isMac || this.isLinux) {
      // NOTE: assuming linux & macos users having adb installed on their systems
      // this will use system's adb executable
      // for example: adb devices -l
      // By doing this we will no longer have to ship adb executable which can
      // also be outdated.
      // This will also allow users to connect to their devices,
      // even before using applciation by wireless debugging.
